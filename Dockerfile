...
ARG BUILD_NEIGHBOR_ORDER_COUNT=128
...
RUN mkdir -p /out/data \
    && if [ -f resources/references.json.gz ]; then \
         gzip -dc resources/references.json.gz | BUILD_BLOCK_INDEX="$BUILD_BLOCK_INDEX" BUILD_NATIVE_ONLY_INDEX="$BUILD_NATIVE_ONLY_INDEX" BUILD_NEIGHBOR_ORDER_COUNT="$BUILD_NEIGHBOR_ORDER_COUNT" /out/app/RinhaFraud build-index /out/data/references.idx ; \
         ...
         curl -fsSL https://raw.githubusercontent.com/zanfranceschi/rinha-de-backend-2026/main/resources/references.json.gz \
           | gzip -dc \
           | BUILD_BLOCK_INDEX="$BUILD_BLOCK_INDEX" BUILD_NATIVE_ONLY_INDEX="$BUILD_NATIVE_ONLY_INDEX" BUILD_NEIGHBOR_ORDER_COUNT="$BUILD_NEIGHBOR_ORDER_COUNT" /out/app/RinhaFraud build-index /out/data/references.idx ; \
       fi \
